...
public static class cuda {
    ...
    public class CudaException : ExternalException {
        public CudaException(string message, CUresult errorCode) :
    ...
    public static void checkCudaErrors(CUresult errorCode) {
        if (errorCode != CUresult.CUDA_SUCCESS) {
            if (cuGetErrorName(errorCode, out IntPtr pStr) == CUresult.CUDA_SUCCESS) {
                throw new CudaException(Marshal.PtrToStringAnsi(pStr), errorCode);
            } else {
                throw new CudaException($"CUDA error: {errorCode}", errorCode);
            }
        }
    }
